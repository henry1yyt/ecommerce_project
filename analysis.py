...
<body>
<header class="hero"><div class="eyebrow">AI-EXTENDED DATA · INTERNSHIP CAPSTONE</div><h1>增长之后，利润与用户价值留下了吗？</h1><p>从课程 demo 的 AI 规则扩展，到数据清洗、经营复盘、同期群、RFM 聚类和退货风险预警。本报告所有指标均由同一条可复现分析链路生成。</p></header>
...
function show(id){document.querySelectorAll('.panel').forEach(x=>x.classList.toggle('active',x.id===id));document.querySelectorAll('.nav button').forEach(x=>x.classList.toggle('active',x.dataset.id===id))}nav.onclick=e=>e.target.dataset.id&&show(e.target.dataset.id);show('overview');
</script>
</body></html>'''
